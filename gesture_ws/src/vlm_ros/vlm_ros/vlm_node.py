...
# -----------------------------------------------------------------------------
# Gesture label space + default prompt
# -----------------------------------------------------------------------------
...
class VLMNode(Node):
    ...
    def _aggregate(self, preds: List[str]) -> Tuple[str, float]:
        """Aggregate frame-wise raw outputs into a single canonical label + confidence."""
        from collections import Counter

        cleaned = [self._canonical_label(p) for p in preds if p]
        if not cleaned:
            return "UNKNOWN", 0.01
        ...
        # confidence = fraction of frames voting for this label (+tiny floor)
        conf = votes / max(1, len(cleaned))
        conf = 0.98 * conf + 0.01  # keep your smoothing
